...
function extractSevenTVLogin(cardEl: Element): string | null {
  const link = cardEl.querySelector('a.seventv-user-card-usertag[href]');
  if (!link) return null;
...
export function detectCardLogin(element: Element): DetectedCard | null {
  ...
  const seventvCard = element.classList.contains('seventv-user-card')
    ? element
    : element.querySelector('.seventv-user-card');
  if (seventvCard) {
